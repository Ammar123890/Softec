...
import fdata from "./adovocacydata";
import { useForm } from "react-hook-form";
import Modelview from "./AdovocacyView"
import ModalAdd from "./AdovocacyModalAdd";
...
const AdovocacyList = () => {
 ...
 useEffect(() => {
  axios.get("http://localhost:5000/api/advocacy/get", {
    headers: {
  ...
  const onFormSubmit = (submitData) => {
    ...
    let submittedData = {
        ...
        date : Date.now(),
        balance: 552222,
        teamleadId: "submitData.teamleadId",
        status: "pending"
    };
  ...
  return (
    <React.Fragment>
      <Head title="Adovocacy "></Head>
      <Content>
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              <BlockTitle page>Adovocacy Activities</BlockTitle>
                ...
                ? currentItems.map((item) => {
                    return (
                      <DataTableItem key={item.id}>
                        ...
                        <DataTableRow  size="lg">
                            <span className="tb-amount">{item.campaignType}</span>
                        </DataTableRow>
                        <DataTableRow size="lg">
                          <span className="tb-lead-sub">{item.audience}</span>
                        </DataTableRow>
                        <DataTableRow size="lg">
                          <span className="tb-lead-sub">{item.teamleadId}</span>
                        </DataTableRow>
                        
                        <DataTableRow  size="sm">
                            <span className="tb-amount">
                          {item.type=="receiving"? "+":"-"} {item.amount} <span>PKR</span>
                          </span>
                        </DataTableRow>
                        <DataTableRow  size="sm">
                          <span className="tb-amount">
        ...
        <Modal isOpen={modal.add} toggle={() => setModal({ add: false })} className="modal-dialog-centered" size="lg">
          <ModalBody>
            ...
            >
             ...
             <ModalAdd  toggle={() => setModal({ add: false })} />
          </ModalBody>
        </Modal>

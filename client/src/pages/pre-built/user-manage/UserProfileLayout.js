...
const UserProfileLayout = () => {
  ...
  useEffect(() => {
    axios.get("http://localhost:5000/api/ngo/get", {
      headers: { Authorization: `Bearer ${localStorage.getItem("accessToken")}` },
    })
      .then((res) => {
        setData(res.data);
        setProfileName(res.data.name); // move the setProfileName inside the then block
      })
      .catch((error) => {
        console.log(error);
      });
              ...
              <div className="card-inner-group">
                <div className="card-inner">
                  <div className="user-card">
                    <UserAvatar text={findUpper(data.name!=null?data.name:"S H")} theme="primary" />
                ...
                <div className="card-inner">
                  <div className="user-account-info py-0">
                    <h6 className="overline-title-alt">Wallet</h6>
                    <div className="user-balance">
                      {data.capital} <small className="currency currency-btc">PKR</small>
                    </div>
                    <div className="user-balance-sub">
                      {/* Locked{" "}
                      <span>
                        15000 <span className="currency currency-btc">PKR</span>
                      </span> */}
                    </div>
                  </div>
                </div>
